...
public class Gua extends View{
    ...
    public Gua(Context context, AttributeSet attrs) {
        ...
        //外圆环的颜色 默认为深紫色
        mRingColor = typedArray.getColor(R.styleable.PercentageRing_ringColor, 0xff0091fc);
        //中间圆的半径 默认为60
        mRadius = typedArray.getInt(R.styleable.PercentageRing_radius, 150);
        //字体颜色 默认为白色
        mTextColor = typedArray.getColor(R.styleable.PercentageRing_textColor, 0xff0091fc);
    ...
    private void init(Context context){
        ...
        mArcPaint.setStrokeWidth((float) (0.25*mRadius));
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mRadius>mCircleX) {
            ...
            mTextPaint.setStrokeWidth((float) (0.015*mRadius));
            //重新设置字号
            mTextPaint.setTextSize(mRadius/3);
            //重新设置外圆环宽度
    ...
    protected void onDraw(Canvas canvas) {
        ...
        //画文字
        canvas.drawText("剩余",mCircleX,mCircleY-35,mTextPaint);
        canvas.drawText(String.valueOf(mCurrentPercent)+"%", mCircleX, mCircleY+mTextSize/4+35, mTextPaint);
        //判断当前百分比是否小于设置目标的百分比
